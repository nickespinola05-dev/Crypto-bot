"""
utils/pnl_tracker.py — Persistent P/L tracking for paper trading.

Tracks daily P/L and all-time P/L that compound across bot restarts.
Stores data in state/pnl_history.json.

Usage:
    from utils.pnl_tracker import PnLTracker
    tracker = PnLTracker()
    tracker.record_equity(979.36)   # call each cycle
    summary = tracker.get_summary()  # read from dashboard
"""
...
def get_summary() -> dict:
    ...
    daily_pnl_pct = (daily_pnl / today_open * 100) if today_open > 0 else 0
